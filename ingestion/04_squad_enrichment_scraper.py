...
def scrape_sofifa_ratings() -> pd.DataFrame:
    ...
    Usa soccerdata.SoFIFA — sem API key, scraping gentil.
    """
    if not HAS_SOCCERDATA:
        logger.error("soccerdata necessário para SoFIFA")
        return pd.DataFrame()

    logger.info("🎮 Coletando ratings SoFIFA...")

    try:
        fifa = sd.SoFIFA()
        all_players = []

        # SoFIFA usa edition year (FIFA 25 = 2025)
        for team_name, aliases in tqdm(WC_2026_TEAMS.items(), desc="SoFIFA teams"):
            sofifa_name = aliases["sofifa"]
            try:
                # Buscar jogadores da seleção nacional
                players = fifa.read_players(team=sofifa_name)

                if players is not None and not players.empty:
                    players["wc_team_name"] = team_name
                    players["source"] = "sofifa_2025"
                    players["collected_at"] = datetime.utcnow().isoformat()
                    all_players.append(players)
                    logger.info(f"  ✅ {team_name}: {len(players)} jogadores")
                else:
                    logger.warning(f"  ⚠️  {team_name}: sem dados")

                time.sleep(REQUEST_DELAY_S)

            except Exception as e:
                logger.error(f"  ❌ {team_name}: {e}")
                time.sleep(REQUEST_DELAY_S * 2)

        if not all_players:
            logger.warning("SoFIFA: nenhum dado coletado")
            return pd.DataFrame()

        df = pd.concat(all_players, ignore_index=True)

        # Normalizar colunas MultiIndex se necessário
        if isinstance(df.columns, pd.MultiIndex):
            df.columns = ["_".join(filter(None, map(str, c))).strip() for c in df.columns]

        # Selecionar colunas relevantes (nomes podem variar por versão)
        col_map = {
            "short_name":   ["short_name", "name", "player_name"],
            ...
            "position":     ["player_positions", "positions", "best_position"],
            "pace":         ["pace", "Pace"],
        ...
        result_cols = {"wc_team_name", "source", "collected_at"}
        for std_name, variants in col_map.items():
            for v in variants:
                if v in df.columns:
                    df[std_name] = df[v]
                    result_cols.add(std_name)
                    break

        output_cols = [c for c in result_cols if c in df.columns]
        df_clean = df[output_cols].copy()

        logger.success(f"SoFIFA: {len(df_clean):,} jogadores coletados")
        return df_clean

    except Exception as e:
        logger.error(f"SoFIFA falhou: {e}")
        return pd.DataFrame()
...
def scrape_national_team_form() -> pd.DataFrame:
    ...
    try:
        # FBref tem dados de seleções via "International"
        fbref = sd.FBref(leagues="International", seasons="2024-25")
        schedule = fbref.read_schedule()

        if schedule is None or schedule.empty:
            logger.warning("FBref: sem schedule internacional — usando fallback")
            return build_form_fallback()

        logger.success(f"FBref: {len(schedule)} jogos internacionais coletados")
        schedule["source"] = "fbref_international"
        schedule["collected_at"] = datetime.utcnow().isoformat()
        schedule.to_parquet(OUTPUT_DIR / "national_team_form_raw.parquet", index=False)
        return schedule

    except Exception as e:
        logger.warning(f"FBref forma falhou: {e} — usando fallback")
        return build_form_fallback()
...
def build_form_fallback() -> pd.DataFrame:
    ...
    df["source"] = "curated_fallback_2024_25"
    df["collected_at"] = datetime.utcnow().isoformat()
